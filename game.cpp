...
bool Game::checkWin(Player player) {
	char playerChar;
	if(player == HUMAN) playerChar = player2;
	else playerChar = player1;
		...
		for(int j = 0; j < 3; j++) {
			if(AIboard[i][j] == '-') {
				...
				if(tempMoveScore <= bestMoveScore) {
                    ...
                    if (tempMoveScore == bestMoveScore && (arc4random()%1)) {
...
void Game::play() {
	...
	while(!checkWin(HUMAN) && !checkWin(AI) && !gameOver()) {
		...
		if(turn % 2 == 0) {
			...
			board[AImove.x][AImove.y] = player1;
...
void Game::playAIvsAI() {
	int turn = 0;
	printBoard();
	while(!checkWin(HUMAN) && !checkWin(AI) && !gameOver()) {
		// human move
		if(turn % 2 == 0) {
            cout << endl << "Computer1 Player Move:" << endl;
			Move AImove = minimax(board);
            
			board[AImove.x][AImove.y] = player2;
            
			if(checkWin(HUMAN)) cout << "Computer1 Player Wins" << endl;
			
		} else {
            
			cout << endl << "Computer2 Player Move:" << endl;
            
			Move AImove = minimax(board);
            
			board[AImove.x][AImove.y] = player1;
            
			if(checkWin(AI)) cout << "Computer2 Player Wins" << endl;
